    ...
    if (lavInspect(fit1, "converged")==F){ 
      ...
      if (is.positive.definite(sat.struct.cov)==F){
        ...
        for(i in 1:num.path.mod){
          ...
          #compute fit indices
          cfi.ci.adj.unstr <- cfi.adj.ci(fit2, Gamma, structured=F)
          
          cfi.ci.adj.str.exp <- cfi.adj.ci(fit2, Gamma, structured=T)
          cfi.ci.adj.unstr.tri <- cfi.adj.ci(fit2, Gamma.tri, structured=F)
          
          cfi.ci.adj.str.exp.tri <- cfi.adj.ci(fit2, Gamma.tri, structured=T)
